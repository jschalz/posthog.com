...
exports.uploadImageToCloudinary = async ({ url, publicId, overwrite, reporter }) => {
    ...
    while (true) {
        try {
            ...
            uploadedImages++
            if (uploadedImages == totalImages || uploadedImages % Math.ceil(totalImages / 100) == 0)
                reporter.info(
            ...
            if (attempts < 3) {
                attempts += 1
                reporter.warn(`An error occurred when uploading ${url} to Cloudinary: ${stringifiedError}`)
            } else {
                reporter.panic(`Unable to upload ${url} to Cloudinary after ${attempts} attempts: ${stringifiedError}`)
            }
        }
    }
}
