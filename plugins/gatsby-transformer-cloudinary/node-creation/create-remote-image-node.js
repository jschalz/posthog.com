...
exports.createRemoteImageNode = async ({
    ...
    if (!reporter) {
        throw Error("`reporter` is a required argument. It's available at `CreateNodeArgs.reporter`.")
    }
    if (!url) {
        reporter.panic(
    ...
    if (!createNode) {
        reporter.panic("`createNode` is a required argument. It's available at `CreateNodeArgs.actions.createNode`.")
    }
    if (!createNodeId) {
        reporter.panic("`createNodeId` is a required argument. It's available at `CreateNodeArgs.createNodeId`.")
    }

    const overwrite = overwriteExisting == null ? getPluginOptions().overwriteExisting : overwriteExisting
