...
const { getAssetAsTracedSvg, getAssetMetadata, getUrlAsBase64Image } = require('./asset-data')

const { _generateCloudinaryAssetSource, createResolveCloudinaryAssetData } = require('./resolve-asset')

const resolveCloudinaryAssetData = createResolveCloudinaryAssetData(gatsbyUtilsMocks)
...
describe('generateCloudinaryAssetSource', () => {
    ...
    it('generated correct source data', () => {
        const result = _generateCloudinaryAssetSource(filename, width, height, format, fit, options)
...
describe('resolveCloudinaryAssetData', () => {
    ...
    describe('when unconforming source', () => {
        it('calls reporter.verbose and returns null for empty source', async () => {
            const source = {}
            const args = {}
            const result = await resolveCloudinaryAssetData(source, args, context, info)
    ...
    describe('when fetched asset data is invalid', () => {
        ...
        it('calls reporter.warn on invalid metadata and returns null', async () => {
            const args = {}
            const result = await resolveCloudinaryAssetData(sourceWithoutMeta, args, context, info)
            expect(getAssetMetadata).toBeCalledTimes(1)
            expect(generateImageData).toBeCalledTimes(0)
            expect(gatsbyUtilsMocks.reporter.warn).toBeCalledTimes(1)
            expect(result).toBe(null)
